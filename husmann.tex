...
\section{The function optim\_sa}
As \code{optim\_sa} shall be able to solve very specific optimization problems, several parameters can be defined by the user. Quality of solution and speed of convergence will thus substantially depend on accurate parametrization. The parameters list is pre-parameterized for loss functions of medium complexity. In the following, we will explain each parameter briefly giving hints for useful specification.
\begin{itemize}
	\item \code{fun}: Loss function to be optimized. The function must depend on a vector of covariates and return one numeric value. There are no assumptions on the behavior of the return. Missing (\code{NA}) or undefined (\code{NaN}) returns are allowed as well.
	\item \code{start}: Vector with initial covariate combination.
\end{itemize}

...
\begin{algorithm}[H]
	...
	\While{\textit{t} $>$ $t_{min}$}{
		\For{i in c(1: $n_{inner}$)}{
			...
			}{calculate Metropolis Probability \textit{M} (Equation \ref{eq:eq1})\\ \eIf{uniformly distributed random number [0,1] $<$ \textit{M}}{store $x_j$ and $f(x_j)$\\$x_{i} \gets x_{i*}; f(x_{i}) \gets f(x_{i*})$}{$x_{i} \gets x_j; f(x_{i}) \gets f(x_j)$}}

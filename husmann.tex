...
As all functions were practically able to minimize equation \ref{eq:eq1}, comparison of calculation times appears to be another important point for quality assessment of the algorithms. It showed up that the direct search method \code{optim NM} was by far faster then the random search methods (Figure \ref{fig:fig1}). The ranking within the group of random search methods was also obvious. \code{GenSA} was in mean 3.5 times faster than \code{optim\_sa}. Both functions able coping with equally valued optima were slower than these returning only 1 solution (Table \ref{tab:tab1}, Figure \ref{fig:fig1}). Due to the multiple additional options (which are all called several times in the code) \code{optim\_sa} was by far slowest. This example clearly reveals that higher flexibility on the one hand leads to significantly higher calculation times on the other hand.
...
A second example illustrating option to flexibly define parameter space also bases on the function by \citet{himmelblau_1972}. In case the user is interested in integer covariate combinations only, the \code{vf} option can be used to restrict the parameter space. The following simple example shows how a variation rule could look like for integer programming problems. Mixed integer problems work similarly.
...
Calling the minimization function always leads to $f(3, 2)=0$. To achieve this, vf must be defined as a R function. The simple \code{var\_fun\_int} from the example returns a vector of integer covariates varying in dimension of \code{vf} around the passed covariates. The \code{fun\_length} is passed explicitly, although it is implicitly given by the length of \code{para\_0}. because it might help simplifying the \code{vf}. \code{rf} and \code{temp} are optionally. If \code{vf} needs no dynamic part, they can separately be disabled by passing \code{NA}. Other restrictions may be defined analogously. \code{vf} can be used to explicitly restrict the parameter space. The generic plot function of the \code{optimization} package helps interpreting the convergence and thus the solution quality. Since the given example is 2-dimensional, a \textit{contour plot} can be created. It becomes obvious that only integer covariate combinations were examined during optimization (Figure \ref{fig:fig2}). Figure \ref{fig:fig2} reveals that though response of iteration 10 with parameter combination \{2, 2\} was already quite near to the optimum, the stochastic component of the algorithm leaded to worse response of iteration 11. Further hints for interpretation can be found in the package documentation.

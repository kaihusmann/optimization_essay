...
We briefly explain where the \code{optim\_sa} function might be advantageous at 3 examples with different complexity.
...
We performed 10,000 optimizations with different packages in order investigate quality and speed. As the problem could be solved with relatively simple direct search methods we also included the Nelder-Mead (NM) optimization method \citep{nelder_1965} which is the default of \code{optim} from the \CRANpkg{stats} package. We parameterized the \code{optim\_sa} function for relatively simple optimization problems. We defined the problem to be sufficiently solved when the response after minimization is < 0.01. We tried to find parameters for all other investigated functions which fulfill the sufficiency definition. We performed optimizations with \CRANpkg{GenSA} and \CRANpkg{stats} having the following parameters:
...
It can be seen that parameterization of NM is, as expected, considerable more easy. The function only needs starting values. With view to \textit{accuracy} each methods performed well. In mean of 10,000 calls all functions returned responses < 0.01 after minimization. Thus all functions are able to solve the problem. Since we have a multimoda problem,
After checking this necessary condition, we checked the efficiency.
